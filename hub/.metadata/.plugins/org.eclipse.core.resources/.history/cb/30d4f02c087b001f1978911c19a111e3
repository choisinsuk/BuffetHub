...
public class Reserve {
	...
	@Column(name= "rs_nb", nullable = false, precision = 8)
	private Integer rs_nb; // 예약번호
	
	@Column(name = "rs_dt", nullable = false)
	private Date rs_dt; // 예약일자
	
	@Column(name = "rs_total_person_cnt", nullable = false, length = 4)
	private Integer rs_total_person_cnt; // 예약 총 인원수
	@Column(name = "rs_adult_person_cnt", nullable = false, length = 4)
	private Integer rs_adult_person_cnt; // 예약 성인 인원수
	@Column(name = "rs_child_person_cnt", nullable = false, length = 4)
	private Integer rs_child_person_cnt; // 예약 아동 인원수
	@Column(name = "rs_preage_person_cnt", nullable = false, length = 4)
	private Integer rs_preage_person_cnt; // 예약 미취학 인원수
	
	@Column(name = "rs_visit_total_cnt", nullable = false, length = 4)
	private Integer rs_visit_total_cnt; // 방문 총 인원수
	@Column(name = "rs_visit_adult_cnt", nullable = false, length = 4)
	private Integer rs_visit_adult_cnt; // 방문 성인 인원수
	@Column(name = "rs_visit_child_cnt", nullable = false, length = 4)
	private Integer rs_visit_child_cnt; // 방문 아동 인원수
	@Column(name = "rs_visit_preage_cnt", nullable = false, length = 4)
	private Integer rs_visit_preage_cnt; // 방문 미취학 인원수
	@Column(name = "rs_realpeople_cnt", nullable = false, length = 4)
