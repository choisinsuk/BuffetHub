...
const NoticeChange = () => {
  ...
  const handlePostCreate = (post) => {
    ...
    navigate("/NoticeChange"); // 수정 페이지로 이동
  };

  // 수정 버튼 클릭 핸들러
  const handleEdit = () => {
    navigate("/NoticeChange"); // NoticeChange 페이지로 이동
  };

  return (
    <BoardLayout>
      <div className="text-5xl text-center">
        <div className="bg-customColor1 w-full px-3 py-20 rounded-lg flex items-center justify-center text-white text-bold">공지사항 수정</div>
        <BoardNav />
...
export default NoticeChange; // Change 컴포넌트 내보내기
