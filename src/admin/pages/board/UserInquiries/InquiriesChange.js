...
const InquiriesChange = () => {
  ...
  const handlePostCreate = (post) => {
    ...
    navigate("/InquiriesChange"); // 수정 페이지로 이동
  };

  // 수정 버튼 클릭 핸들러 (InquiriesChange 페이지로 이동)
  const handleEdit = () => {
    navigate("/InquiriesChange"); // InquiriesChange 페이지로 이동
  };

  return (
    <BoardLayout>
      <div className="text-5xl text-center">
        <div className="bg-customColor1 w-full px-3 py-20 rounded-lg flex items-center justify-center text-white text-bold">고객문의 수정</div>
        <BoardNav />
        ...
        <div className="mt-4 flex justify-center space-x-2">
          ...
          <button
            type="button"
            onClick={handleInquiry} // 작성 버튼 클릭 시 호출
...
export default InquiriesChange; // InquiriesChange 컴포넌트 내보내기
